...
Has the mandatory seat belt policy been statistically effective since 1983?

# Data Analysis
## Analysis of trends in driverkilled before and after 1983

```{r figure1, fig.cap="Line graph for people killed}
selectdata <- Seatbelts %>% 
  select(DriversKilled, drivers, front, rear, Year)

ggplot(data = selectdata) +
  geom_smooth(mapping = aes(x = Year, y = DriversKilled))->f1

ggplot(data = selectdata) +
  geom_smooth(mapping = aes(x = Year, y = drivers))->f2

ggplot(data = selectdata) +
  geom_smooth(mapping = aes(x = Year, y = front))->f3

ggplot(data = selectdata) +
  geom_smooth(mapping = aes(x = Year, y = rear))->f4
