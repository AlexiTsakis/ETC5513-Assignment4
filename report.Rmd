...
seatbelts
fit<-lm(DriversKilled~., data=seatbelts)
summary(fit)
```
...
ggplot(aug, aes(x=.fitted, y=.resid))+
  geom_point()+
  geom_smooth()->res
res+geom_hline(yintercept=0, color="red")+
  ggtitle("residuals VS fitted value")
```
From the figure \@ref(fig:residuals), it indicates that the residuals does not include any pattern of the variables since the residuals are randomly around the red line. It means that the model can capture all the information about the respinse variable DriversKilled. But we can see that the tail at the beginning and end is not like a straingt since the obervations are not enough at the end and the beginning.
{Section 3}
